    ...
    try:
        for test in TESTS:
            if not limit or any(l in test.title.lower() for l in limit):
                # Killing QEMU may sometimes corrupt NvVars in ESP directory.
                # Make sure a clean run is issued when running tests.
                if os.path.isdir("LoaderPkg/ESP"):
                    shutil.rmtree("LoaderPkg/ESP")
...
""")
    ...
    def user_test(self, binary, *monitors, **kw):
        ...
        if kw.pop("snapshot", True):
            kw.setdefault("make_args", []).append("CONFIG_SNAPSHOT=y")
...
def get_symbol_address(s):
    addrs = [int(sym[:16], 16) for sym in open("obj/kern/kernel.sym")
            if sym[19:].strip() == s]
